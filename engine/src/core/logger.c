...
void log_output(log_level level, const char* message, ...){
    ...
    // Technically imposes a 32k charactrer limit on a single log entry, but....
    // DON'T DO THAT!
    const i32 msg_legth = 32000;
    char out_message[msg_legth];
    ...
    vsnprintf(out_message, msg_legth, message, arg_ptr);
    va_end(arg_ptr);

    char out_message2[msg_legth];
